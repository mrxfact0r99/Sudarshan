...
CLIPBOARD_KEYWORD_HINTS = [
    ...
    "bearer ", "access key", "private key", "credential", "ssn",
    "social security", "credit card", "card number", "cvv", "cvv2",
    "bank account", "routing number", "iban", "seed phrase", "mnemonic",
    "wallet", "otp", "one-time code", "2fa", "pin code",
]

CLIPBOARD_PATTERNS = [
    ("Email address", "High",
     re.compile(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}")),
    ("Possible credit card number", "High",
     re.compile(r"\b(?:\d[ -]?){13,19}\b")),
    ("AWS access key ID", "High",
     re.compile(r"\bAKIA[0-9A-Z]{16}\b")),
    ("Private key block", "High",
     re.compile(r"-----BEGIN [A-Z0-9 ]*PRIVATE KEY-----")),
    ("Possible JWT (JSON Web Token)", "Medium",
...
BRAND_NAVY = colors.HexColor("#152238")       
BRAND_ACCENT = colors.HexColor("#2E5C8A")     
BRAND_HEADER_BG = colors.HexColor("#152238")  
BRAND_MUTED = colors.HexColor("#5B6472")      
BRAND_RULE = colors.HexColor("#D6DAE1")      
BRAND_ZEBRA = colors.HexColor("#F4F6F9")      
BRAND_BORDER = colors.HexColor("#C7CCD4")    
...
def analyze(processes, connections, usb_events, usb_note, login_events, login_note,
    ...
    if not clipboard_data:
        findings.append({
            ...
            "category": "Collection gap - no clipboard data captured",
...
def risk_label_cell(risk, cell_style=None, chip_styles=None):

    if chip_styles and risk in chip_styles:
        style = chip_styles[risk]
    else:
        bg = SEVERITY_COLOR.get(risk, SEVERITY_COLOR["Clean"])
        style = ParagraphStyle(
            f"Chip{risk}", fontName="Helvetica-Bold", fontSize=6.1, leading=7.6,
            textColor=colors.white, alignment=TA_CENTER, backColor=bg,
            borderPadding=(2, 2, 2, 2),
        )
    return Paragraph(risk.upper(), style)


def build_severity_chip_styles():
    out = {}
    for sev, bg in SEVERITY_COLOR.items():
        out[sev] = ParagraphStyle(
            f"Chip{sev}", fontName="Helvetica-Bold", fontSize=6.1, leading=7.6,
            textColor=colors.white, alignment=TA_CENTER, backColor=bg,
            borderPadding=(2, 2, 2, 2),
        )
    return out
...
def kpi_card(value, label, accent_color, width=1.28 * inch):
    num_style = ParagraphStyle("KpiNum", fontName="Helvetica-Bold", fontSize=19,
                                leading=22, textColor=BRAND_NAVY, alignment=TA_CENTER)
    lbl_style = ParagraphStyle("KpiLbl", fontName="Helvetica-Bold", fontSize=6.6,
                                leading=8.5, textColor=BRAND_MUTED, alignment=TA_CENTER)
    t = Table([[Paragraph(str(value), num_style)],
               [Paragraph(label.upper(), lbl_style)]], colWidths=[width])
    t.setStyle(TableStyle([
        ("BOX", (0, 0), (-1, -1), 0.75, BRAND_BORDER),
        ("LINEABOVE", (0, 0), (-1, 0), 2.6, accent_color),
...
def kpi_card_row(cards, gap=0.14 * inch):
    row = [[]]
    widths = []
    for i, c in enumerate(cards):
        row[0].append(c)
        widths.append(c._colWidths[0] if hasattr(c, "_colWidths") else 1.28 * inch)
    outer = Table(row, colWidths=None, hAlign="LEFT")
...
def build_severity_pie(sev_counts, size=1.9 * inch):
    order = ["High", "Medium", "Low", "Info"]
    data = [max(sev_counts.get(k, 0), 0) for k in order]
    labels = [f"{k} ({sev_counts.get(k, 0)})" for k in order]

    d = Drawing(size + 1.7 * inch, size + 0.1 * inch)
    if sum(data) == 0:
        d.add(String(4, size / 2, "No findings flagged", fontName="Helvetica",
                      fontSize=8, fillColor=BRAND_MUTED))
        return d
    ...
    for i, sev in enumerate(order):
        pie.slices[i].fillColor = SEVERITY_COLOR[sev]
    d.add(pie)
    ...
    legend.colorNamePairs = [(SEVERITY_COLOR[order[i]], labels[i]) for i in range(4)]
    d.add(legend)
    return d


def build_category_bar(category_counts, size_w=4.55 * inch, size_h=1.9 * inch):
    labels = [c[0] for c in category_counts]
    values = [c[1] for c in category_counts]

    d = Drawing(size_w, size_h + 0.35 * inch)
    if sum(values) == 0:
        d.add(String(4, size_h / 2, "No flagged activity in any category", fontName="Helvetica",
                      fontSize=8, fillColor=BRAND_MUTED))
        return d
    ...
    chart.x = 30
    chart.y = 35
    chart.width = size_w - 45
    chart.height = size_h - 10
    chart.data = [values]
    ...
    chart.valueAxis.valueStep = max(1, -(-max(max_v, 1) // 4))
...
def make_table(data, col_widths, header_bg=BRAND_HEADER_BG):
    ...
    t.setStyle(TableStyle([
        ...
        ("INNERGRID", (0, 1), (-1, -1), 0.35, BRAND_RULE),
        ("BOX", (0, 0), (-1, -1), 0.75, BRAND_BORDER),
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, BRAND_ZEBRA]),
...
def generate_pdf(output_dir, case_name, examiner, evidence_source,
    ...
    title_style = ParagraphStyle("TitleBig", parent=styles["Title"], fontSize=25,
                                  leading=30, alignment=TA_CENTER, textColor=BRAND_NAVY,
                                  fontName="Helvetica-Bold")
    kicker_style = ParagraphStyle("Kicker", parent=styles["Normal"], fontSize=9.5,
                                   alignment=TA_CENTER, textColor=BRAND_ACCENT,
                                   fontName="Helvetica-Bold", spaceAfter=4)
    subtitle_style = ParagraphStyle("Subtitle", parent=styles["Normal"], fontSize=12,
                                     alignment=TA_CENTER, textColor=BRAND_MUTED,
                                     spaceAfter=6)
    ...
    chip_styles = build_severity_chip_styles()
    cover_label = ParagraphStyle("CoverLabel", parent=styles["Normal"], fontSize=9,
                                  leading=13, fontName="Helvetica-Bold", textColor=BRAND_NAVY)
    cover_value = ParagraphStyle("CoverValue", parent=styles["Normal"], fontSize=9,
                                  leading=13, textColor=colors.HexColor("#232A35"))

    def section_rule(space_after=10):
        """Thin accent rule placed under an H1 heading for a cleaner section break."""
        return [
            HRFlowable(width="100%", thickness=1.1, color=BRAND_ACCENT,
    ...
    def build_pdf_part(filename, part_story, part_label):
        ...
        part_doc = SimpleDocTemplate(
            ...
            title=f"Digital Forensic Report - {case_name} - {part_label}",
        )
        ...
        def add_page_number(canvas_obj, doc_obj):
            ...
            if doc_obj.page > 1:
                ...
                canvas_obj.drawString(0.6 * inch, page_h - 0.42 * inch, str(case_name))
                canvas_obj.setFont("Helvetica", 8)
                canvas_obj.setFillColor(BRAND_MUTED)
                canvas_obj.drawRightString(page_w - 0.6 * inch, page_h - 0.42 * inch,
                                            "DIGITAL FORENSIC REPORT")
                canvas_obj.setStrokeColor(BRAND_RULE)
                canvas_obj.setLineWidth(0.6)
                canvas_obj.line(0.6 * inch, page_h - 0.5 * inch,
                                 page_w - 0.6 * inch, page_h - 0.5 * inch)

            canvas_obj.setStrokeColor(BRAND_RULE)
            canvas_obj.setLineWidth(0.6)
            canvas_obj.line(0.6 * inch, 0.52 * inch, page_w - 0.6 * inch, 0.52 * inch)
            canvas_obj.setFont("Helvetica", 7.5)
            canvas_obj.setFillColor(BRAND_MUTED)
            canvas_obj.drawString(0.6 * inch, 0.36 * inch, str(part_label))
            canvas_obj.drawRightString(page_w - 0.6 * inch, 0.36 * inch,
                                        f"Page {doc_obj.page}")
    ...
    def part_header(title_text):
        return [
            ...
            Spacer(1, 0.08 * inch),
            HRFlowable(width="100%", thickness=0.75, color=BRAND_RULE,
    ...
    story.append(Paragraph("•&nbsp; DIGITAL FORENSIC &amp; REPORT", kicker_style))
    ...
    story.append(HRFlowable(width="35%", thickness=1.4, color=BRAND_ACCENT,
                             spaceBefore=0, spaceAfter=0.35 * inch, hAlign="CENTER"))
    ...
    cover_data_wrapped = [
        [Paragraph(safe(label), cover_label), Paragraph(safe(value), cover_value)]
        for label, value in cover_data
    ]
    cover_table = Table(cover_data_wrapped, colWidths=[2.1 * inch, 4.3 * inch])
    cover_table.setStyle(TableStyle([
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ("BACKGROUND", (0, 0), (0, -1), colors.HexColor("#EEF1F6")),
        ("LINEBELOW", (0, 0), (-1, -2), 0.4, BRAND_RULE),
        ("BOX", (0, 0), (-1, -1), 0.75, BRAND_BORDER),
    ...
    story.append(Paragraph("ABOUT THIS REPORT", ParagraphStyle(
        "AboutHdr", parent=small, fontName="Helvetica-Bold", textColor=BRAND_NAVY,
    ...
    EXEC_SUMMARY_CATEGORIES = [
        ...
        ("clipboard", "Clipboard"),
        ("command_history", "Cmd History"),
        ("executed_programs", "Exec. Programs"),
    ]
    EXEC_SUMMARY_LABELS = [
    ...
    def _cat_count(target_key):
        items = by_target.get(target_key, [])
        if target_key in ("process", "eventlog"):
            items = [f for f in items if f["severity"] in ("High", "Medium")]
        return len(items)

    category_counts = [(lbl, _cat_count(key)) for key, lbl in EXEC_SUMMARY_CATEGORIES]
    category_counts.sort(key=lambda x: x[1], reverse=True)

    story.append(kpi_card_row([
        kpi_card(sev_counts.get("High", 0), "High Severity", SEVERITY_COLOR["High"]),
        kpi_card(sev_counts.get("Medium", 0), "Medium Severity", SEVERITY_COLOR["Medium"]),
        kpi_card(sev_counts.get("Low", 0), "Low Severity", SEVERITY_COLOR["Low"]),
        kpi_card(sev_counts.get("Info", 0), "Info / Data-Quality", SEVERITY_COLOR["Info"]),
        kpi_card(len(findings), "Total Flagged Items", BRAND_NAVY),
    ]))
    story.append(Spacer(1, 0.22 * inch))

    story.append(Paragraph("Findings Overview", ParagraphStyle(
        "FindOverview", parent=body, fontName="Helvetica-Bold", fontSize=10,
        textColor=BRAND_NAVY, spaceAfter=6)))
    charts_row = Table(
        [[build_severity_pie(sev_counts), build_category_bar(category_counts)]],
        colWidths=[2.75 * inch, 4.55 * inch],
    )
    ...
    story.append(Paragraph("Data Collection Summary", ParagraphStyle(
        "DataCollHdr", parent=body, fontName="Helvetica-Bold", fontSize=10,
        textColor=BRAND_NAVY, spaceAfter=6)))
    ...
    EXEC_SUMMARY_CATEGORIES = EXEC_SUMMARY_LABELS
    ...
    for target_key, label in EXEC_SUMMARY_CATEGORIES:
        ...
        if not cat_findings:
            ...
            for f in cat_findings:
                cat_rows.append([
                    risk_label_cell(f["severity"], chip_styles=chip_styles),
                    Paragraph(safe(f["category"]), cell),
                    Paragraph(safe(f["pid"]), cell),
                    Paragraph(safe(f["detail"]), cell),
                ])
            story.append(make_table(
                cat_rows, col_widths=[0.55 * inch, 1.5 * inch, 0.55 * inch, 4.2 * inch],
    ...
    if not findings:
        ...
        story.append(make_table(rows, col_widths=[0.55 * inch, 1.5 * inch, 0.5 * inch, 4.25 * inch]))

    story = proc_story
    story.extend(part_header("Digital Forensic Report — Process Inventory (Part 2 of 11)"))
    ...
    for p in processes:
        ...
        proc_rows.append([
            risk_label_cell(risk, chip_styles=chip_styles),
    ...
    story.append(make_table(
        proc_rows,
        col_widths=[0.6 * inch, 0.5 * inch, 0.4 * inch, 0.9 * inch, 0.6 * inch, 1.4 * inch, 1.8 * inch]
    ))

    story = net_story
    story.extend(part_header("Digital Forensic Report — Network Connection Inventory (Part 3 of 11)"))
    ...
    story.append(make_table(
        net_rows,
        col_widths=[0.6 * inch, 0.45 * inch, 0.95 * inch, 0.6 * inch, 1.5 * inch, 1.5 * inch, 0.6 * inch]
    ))

    story = usb_story
    story.extend(part_header("Digital Forensic Report — USB &amp; Login Event Inventory (Part 4 of 11)"))
    ...
    if not usb_events:
        ...
        story.append(make_table(
            usb_rows,
            col_widths=[0.6 * inch, 1.3 * inch, 0.9 * inch, 1.0 * inch, 0.5 * inch, 0.65 * inch, 1.3 * inch]
        ))
    ...
    if not login_events:
        ...
        story.append(make_table(
            login_rows,
            col_widths=[0.6 * inch, 0.9 * inch, 0.6 * inch, 0.8 * inch, 1.1 * inch, 0.65 * inch, 1.2 * inch]
        ))
    story.append(Spacer(1, 0.15 * inch))
    ...
    if not usbstor_devices:
        ...
        story.append(make_table(
            stor_rows,
            col_widths=[0.6 * inch, 1.9 * inch, 2.55 * inch, 1.6 * inch]
        ))
    story.append(Spacer(1, 0.15 * inch))
    ...
    if not file_write_events:
        ...
        story.append(make_table(
            fw_rows,
            col_widths=[0.6 * inch, 0.8 * inch, 2.55 * inch, 1.1 * inch, 1.55 * inch]
        ))
    story.append(Spacer(1, 0.1 * inch))
    ...
    story.extend(part_header("Digital Forensic Report — Windows Event Log Review (Part 5 of 11)"))
    ...
    story = browser_story
    story.extend(part_header("Digital Forensic Report — Browser History Review (Part 6 of 11)"))
    ...
    story = corr_story
    story.extend(part_header("Digital Forensic Report — Process-to-Network Correlation (Part 7 of 11)"))
    ...
    story.extend(part_header("Digital Forensic Report — Recycle Bin &amp; Deleted File Review (Part 8 of 11)"))
    ...
    if not recycle_bin_path:
        ...
        if not recycle_bin_items:
            ...
            story.append(make_table(
                all_rows,
                col_widths=[0.55 * inch, 1.8 * inch, 0.65 * inch, 1.0 * inch, 0.6 * inch, 0.8 * inch, 1.15 * inch]
            ))
    ...
    story.extend(part_header("Digital Forensic Report — Clipboard Content Review (Part 9 of 11)"))
    story.append(Paragraph("10. Clipboard Content Review", h1))
    story.extend(section_rule())
    clip_content = clipboard_data.get("content", "")
    clip_error = clipboard_data.get("error")
    if not clipboard_path:
        story.append(Paragraph("No clipboard export was present in the evidence file.", body))
    else:
        story.append(Paragraph(
            f"Read method: {safe(clipboard_meta.get('read_method', 'Unknown'))}. "
            f"Content length at collection time: "
            f"{safe(clipboard_data.get('content_length', len(clip_content)))} character(s).",
            body))
        story.append(Spacer(1, 0.1 * inch))

        clip_findings = [f for f in findings if f["target"] == "clipboard" and f["severity"] != "Info"]
        story.append(Paragraph(f"10.1 Flagged Clipboard Indicators ({len(clip_findings)})", h2))
        if clip_error:
            story.append(Paragraph(f"Clipboard capture reported an error: {safe(clip_error)}", body))
        elif not clip_findings:
            story.append(Paragraph(
                "No credential-related keyword or sensitive-data pattern was matched "
                "in the captured clipboard content.", body))
        else:
            cf_rows = [["Sev.", "Category", "Detail"]]
            for f in sorted(clip_findings, key=lambda f: SEVERITY_ORDER.get(f["severity"], 9)):
                cf_rows.append([
                    risk_label_cell(f["severity"], chip_styles=chip_styles),
                    Paragraph(safe(f["category"]), cell),
                    Paragraph(safe(f["detail"]), cell),
                ])
            story.append(make_table(cf_rows, col_widths=[0.5 * inch, 2.3 * inch, 4.6 * inch]))
        story.append(Spacer(1, 0.15 * inch))

        story.append(Paragraph("10.2 Full Captured Clipboard Content", h2))
        story.append(Paragraph(
            "Reproduced verbatim below for examiner review. Treat this content with "
    ...
    story = cmdhist_story
    story.extend(part_header("Digital Forensic Report — Command History Review (Part 10 of 11)"))
    ...
    story.extend(part_header("Digital Forensic Report — Recently Executed Programs Review (Part 11 of 11)"))
    ...
    if not executed_programs_path:
        ...
        if not ep_flagged:
            ...
            story.append(make_table(
                ef_rows, col_widths=[0.55 * inch, 0.85 * inch, 1.1 * inch, 2.4 * inch, 0.7 * inch, 1.15 * inch]
            ))
        story.append(Spacer(1, 0.15 * inch))
    ...
    story.append(Spacer(1, 0.08 * inch))
    story.append(Paragraph(
        "<b>Clipboard keyword/pattern matching:</b> Section 2/10 flags are a coarse "
        "triage heuristic based on keywords (e.g. 'password', 'api key') and simple "
        "patterns (email addresses, card-number-shaped digit runs, private-key "
        "headers, AWS access key IDs, JWT-shaped tokens) found in the single "
        "clipboard snapshot captured at collection time. Matches can be false "
        "positives (e.g. a long phone number matching the card-number pattern), and "
        "the clipboard only reflects the last item copied - it does not capture "
    ...
    story.append(Paragraph(
        ...
        "collection time). Clipboard evidence is a single point-in-time snapshot only - "
...
def main():
    ...
    case_name = os.environ.get("SUDARSHAN_CASE_NAME", "").strip() or "Anonymous"
    examiner = os.environ.get("SUDARSHAN_EXAMINER", "").strip() or "Anonymous"
    ...
    if clipboard_path:
        clipboard_data = load_clipboard_json(clipboard_path)
        if clipboard_data.get("error"):
